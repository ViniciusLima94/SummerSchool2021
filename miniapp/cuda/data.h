...
class Field {
    ...
    void update_host() {
        ...
        cudaMemcpy(host_ptr_, device_ptr_, length()*sizeof(double), cudaMemcpyDeviceToHost);
    }

    void update_device() {
        // TODO: copy contents of host_ptr_ to device_ptr_ 
        cudaMemcpy(host_ptr_, device_ptr_, length()*sizeof(double), cudaMemcpyHostToDevice);
    }
